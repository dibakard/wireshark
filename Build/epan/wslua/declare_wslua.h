/* This file is automatically generated by make-reg.py; do not edit! */

#define WSLUA_DECLARE_CLASSES() \
	WSLUA_CLASS_DECLARE(Address);\
	WSLUA_CLASS_DECLARE(ByteArray);\
	WSLUA_CLASS_DECLARE(CaptureInfo);\
	WSLUA_CLASS_DECLARE(CaptureInfoConst);\
	WSLUA_CLASS_DECLARE(Column);\
	WSLUA_CLASS_DECLARE(Columns);\
	WSLUA_CLASS_DECLARE(Conversation);\
	WSLUA_CLASS_DECLARE(Dir);\
	WSLUA_CLASS_DECLARE(Dissector);\
	WSLUA_CLASS_DECLARE(DissectorTable);\
	WSLUA_CLASS_DECLARE(PseudoHeader);\
	WSLUA_CLASS_DECLARE(Dumper);\
	WSLUA_CLASS_DECLARE(FieldInfo);\
	WSLUA_CLASS_DECLARE(Field);\
	WSLUA_CLASS_DECLARE(File);\
	WSLUA_CLASS_DECLARE(FileHandler);\
	WSLUA_CLASS_DECLARE(FrameInfo);\
	WSLUA_CLASS_DECLARE(FrameInfoConst);\
	WSLUA_CLASS_DECLARE(ProgDlg);\
	WSLUA_CLASS_DECLARE(TextWindow);\
	WSLUA_CLASS_DECLARE(Int64);\
	WSLUA_CLASS_DECLARE(UInt64);\
	WSLUA_CLASS_DECLARE(Listener);\
	WSLUA_CLASS_DECLARE(NSTime);\
	WSLUA_CLASS_DECLARE(PrivateTable);\
	WSLUA_CLASS_DECLARE(Pinfo);\
	WSLUA_CLASS_DECLARE(Pref);\
	WSLUA_CLASS_DECLARE(Prefs);\
	WSLUA_CLASS_DECLARE(Proto);\
	WSLUA_CLASS_DECLARE(ProtoExpert);\
	WSLUA_CLASS_DECLARE(ProtoField);\
	WSLUA_CLASS_DECLARE(Struct);\
	WSLUA_CLASS_DECLARE(TreeItem);\
	WSLUA_CLASS_DECLARE(Tvb);\
	WSLUA_CLASS_DECLARE(TvbRange);\
	WSLUA_CLASS_DECLARE(GcryptCipher);\


#define WSLUA_DECLARE_FUNCTIONS() \
	WSLUA_FUNCTION wslua_all_field_infos(lua_State* L);\
	WSLUA_FUNCTION wslua_register_filehandler(lua_State* L);\
	WSLUA_FUNCTION wslua_deregister_filehandler(lua_State* L);\
	WSLUA_FUNCTION wslua_gui_enabled(lua_State* L);\
	WSLUA_FUNCTION wslua_register_menu(lua_State* L);\
	WSLUA_FUNCTION wslua_register_packet_menu(lua_State* L);\
	WSLUA_FUNCTION wslua_new_dialog(lua_State* L);\
	WSLUA_FUNCTION wslua_retap_packets(lua_State* L);\
	WSLUA_FUNCTION wslua_copy_to_clipboard(lua_State* L);\
	WSLUA_FUNCTION wslua_open_capture_file(lua_State* L);\
	WSLUA_FUNCTION wslua_get_filter(lua_State* L);\
	WSLUA_FUNCTION wslua_set_filter(lua_State* L);\
	WSLUA_FUNCTION wslua_get_color_filter_slot(lua_State* L);\
	WSLUA_FUNCTION wslua_set_color_filter_slot(lua_State* L);\
	WSLUA_FUNCTION wslua_apply_filter(lua_State* L);\
	WSLUA_FUNCTION wslua_reload(lua_State* L);\
	WSLUA_FUNCTION wslua_reload_packets(lua_State* L);\
	WSLUA_FUNCTION wslua_redissect_packets(lua_State* L);\
	WSLUA_FUNCTION wslua_reload_lua_plugins(lua_State* L);\
	WSLUA_FUNCTION wslua_browser_open_url(lua_State* L);\
	WSLUA_FUNCTION wslua_browser_open_data_file(lua_State* L);\
	WSLUA_FUNCTION wslua_register_postdissector(lua_State* L);\
	WSLUA_FUNCTION wslua_dissect_tcp_pdus(lua_State* L);\
	WSLUA_FUNCTION wslua_get_version(lua_State* L);\
	WSLUA_FUNCTION wslua_set_plugin_info(lua_State* L);\
	WSLUA_FUNCTION wslua_format_date(lua_State* L);\
	WSLUA_FUNCTION wslua_format_time(lua_State* L);\
	WSLUA_FUNCTION wslua_get_preference(lua_State* L);\
	WSLUA_FUNCTION wslua_set_preference(lua_State* L);\
	WSLUA_FUNCTION wslua_reset_preference(lua_State* L);\
	WSLUA_FUNCTION wslua_apply_preferences(lua_State* L);\
	WSLUA_FUNCTION wslua_report_failure(lua_State* L);\
	WSLUA_FUNCTION wslua_dofile(lua_State* L);\
	WSLUA_FUNCTION wslua_loadfile(lua_State* L);\
	WSLUA_FUNCTION wslua_register_stat_cmd_arg(lua_State* L);\
	WSLUA_FUNCTION wslua_wtap_file_type_subtype_description(lua_State* L);\
	WSLUA_FUNCTION wslua_wtap_file_type_subtype_name(lua_State* L);\
	WSLUA_FUNCTION wslua_wtap_name_to_file_type_subtype(lua_State* L);\
	WSLUA_FUNCTION wslua_wtap_pcap_file_type_subtype(lua_State* L);\
	WSLUA_FUNCTION wslua_wtap_pcap_nsec_file_type_subtype(lua_State* L);\
	WSLUA_FUNCTION wslua_wtap_pcapng_file_type_subtype(lua_State* L);\
	WSLUA_FUNCTION wslua_gcry_cipher_algo_info(lua_State* L);\
	WSLUA_FUNCTION wslua_gcry_cipher_algo_name(lua_State* L);\
	WSLUA_FUNCTION wslua_gcry_cipher_map_name(lua_State* L);\
	WSLUA_FUNCTION wslua_gcry_cipher_mode_from_oid(lua_State* L);\
	WSLUA_FUNCTION wslua_gcry_cipher_get_algo_keylen(lua_State* L);\
	WSLUA_FUNCTION wslua_gcry_cipher_get_algo_blklen(lua_State* L);\


extern void wslua_register_classes(lua_State* L);
extern void wslua_register_functions(lua_State* L);


